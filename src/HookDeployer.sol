...
contract HookDeployer is Ownable2StepUpgradeable {
    ...
    function initialize(address factoryAddress) external initializer nonZeroAddress(factoryAddress) {
    ...
    function findSalt(address _nonProfitOrgOwner) external view nonZeroAddress(_nonProfitOrgOwner) returns (bytes32) {
        ...
        address quoter = factory.quoter();
        address stateView = factory.stateView();
